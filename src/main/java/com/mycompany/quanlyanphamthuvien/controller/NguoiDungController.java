...
public class NguoiDungController {
    ...
    public void xuLyTra(String idSachBao, String mssv) {
        ...
        if (found) {
            ...
            JOptionPane.showMessageDialog(userview, "Không tìm thấy thông tin mượn phù hợp!");
        }
    }

    public void hienThiDanhSachDangMuon(String mssv) {
        List<MuonTra> danhSachMuonTra = new ArrayList<>();

        for (MuonTra mt : qlMuonTra.getDanhSachMuonTra()) {
            if (mt.getMssv().equals(mssv)) {
                danhSachMuonTra.add(mt);
            }
        }

        if (danhSachMuonTra.isEmpty()) {
            JOptionPane.showMessageDialog(userview, "Bạn chưa mượn ấn phẩm nào!");
            return;
        }

        DefaultTableModel model = new DefaultTableModel();
        model.addColumn("ID");
        model.addColumn("Tên ấn phẩm");
        ...
        for (MuonTra mt : danhSachMuonTra) {
            String ngayTra = mt.getNgayTra() != null ? dateFormat.format(mt.getNgayTra()) : "Chưa trả";
            String trangThai = mt.isDaTra() ? "Đã trả" : "Đang mượn";
